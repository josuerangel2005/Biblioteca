...
public class BookRepositoryJPA implements BookRepository {
  ...
  public Book saveBook(BookSave bookSave) {
    ...
    return this.bookMapper.toBook(this.libroCrudRepository.save(toSave));
  }
  ...
  public Book updateBook(BookUpdate bookUpdate, int id) {
    ...
    Libro toUpdate = this.libroCrudRepository.findById(id).orElseThrow(() -> new RuntimeException());

    this.bookUpdateMapper.updateLibroFromBookSave(bookUpdate, toUpdate);

    toUpdate.getLibroAutores().forEach(libroAutor -> libroAutor.setLibro(toUpdate));
    toUpdate.getLibroCategorias().forEach(libroCategoria -> libroCategoria.setLibro(toUpdate));

    return this.bookMapper.toBook(this.libroCrudRepository.save(toUpdate));
  }
